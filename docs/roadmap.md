...
Bloc 0 (foundations), Bloc 2 (discreet like/match), Bloc 1 (QR check-in + live presence) and Bloc 3 (match-gated chat) are shipped. Aymane's original scaffold (generic *signup → global directory → open DM*, violating invariants 3 and 1) has been replaced: the data model, RLS, auth-backed identity, the core like/match mechanic, real venue presence and match-only chat exist. Anonymous sign-in is enabled on the project. Safety controls (Bloc 4) are next; the first test on a real night (with a real venue and people) is Bloc 5.
...
- `app/v/[venueSlug]/page.tsx`: the live room. Scanning checks you in (`check_in` RPC), the grid shows only mutually-compatible people **checked in here now**, discreet like, realtime match reveal, realtime presence (room fills/empties live), active match chat entry points, explicit "leave" + re-join.
- `app/chat/[matchId]/page.tsx`: realtime chat for a single match. The route is keyed by `matchId`, never by recipient profile, so there is still no open DM surface.
...
- DB: `presence` is live; `check_in()` RPC; `profiles`/`presence` SELECT scoped to co-present (+ matched) users; `messages` insert/select gated by match membership; `matches.expires_at`; `private.*` RLS helpers; `bartap-close-ended-nights` pg_cron job closing the room and deleting expired matches/messages at 06:00 local.

**Known gaps and debt (addressed during Phase 1):**
- Bloc 3 review follow-up before real testing: likes are not yet expired/night-scoped with matches. Stale likes can survive across nights at the same venue, disable the Like button, conflict on insert, or create a next-night match from a previous-night like. Fix by deleting or night-scoping likes together with expired matches.
- Bloc 3 review follow-up before real testing: message sends must be blocked as soon as `matches.expires_at` has passed, not only after the cron deletes the match. Re-check expiry in the chat send path and enforce it in message RLS so an open tab cannot write during the post-expiry/pre-cleanup window.
...
- **Bloc 3, Chat gated by match** (done, with review follow-up before real testing): chat opens only from an active match row (`/chat/[matchId]`), messages are realtime and structurally tied to `match_id`, and `matches.expires_at` makes match/chat cleanup part of the existing 06:00-local cron. Follow-up: expire/night-scope likes together with matches, and enforce `expires_at` on message writes before cron cleanup.
