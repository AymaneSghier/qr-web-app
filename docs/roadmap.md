...
- DB: `presence` is live with `is_visible`; `check_in()` RPC; `profiles`/`presence` SELECT scoped to co-present visible (+ matched) users and excludes blocked pairs; `messages` insert/select gated by active match membership and blocks; `matches.expires_at`; `blocks` close existing likes/matches/chats; `reports` store reason + optional note; `private.*` RLS helpers; `bartap-close-ended-nights` pg_cron job closing the room and deleting expired matches/messages at 06:00 local.
